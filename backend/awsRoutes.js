...
awsRoutes.route("/images").post(verifyToken, async (request, response) => {
    try {
        ...
        response.status(500).json({ message: "Failed to upload image", error: error.message })
    }
})
...
awsRoutes.route("/images/:id").delete(verifyToken, async (request, response) => {
    ...
    try {
        ...
        response.status(500).json({ message: "Failed to delete image", error: error.message });
    }
});
