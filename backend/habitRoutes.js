...
habitRoutes.route("/habits/:id").get(verifyToken, async (request, response) => {
    let db = database.getDb()
    let data = await db.collection("habits").findOne({_id: new ObjectId(request.params.id) })
...
habitRoutes.route("/habits/:id").put(verifyToken, async (request, response) => {
    ...
    let data = await db.collection("habits").updateOne({_id: new ObjectId(request.params.id)}, mongoObject)
    response.json(data)
})

//#5 - Delete One
//http://localhost:3000/habits/12345
habitRoutes.route("/habits/:id").delete(verifyToken, async (request, response) => {
    let db = database.getDb()
    let data = await db.collection("habits").deleteOne({_id: new ObjectId(request.params.id) })
    response.json(data)
})
...
function verifyToken(request, response, next) {
    ...
    if (!token) {
        return response.status(401).json({ message: "Token is missing or not provided" });
    }
